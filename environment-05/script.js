...
window.addEventListener("DOMContentLoaded", () => {
  ...
  fetch("runners.json")
    ...
    .then((data) => {
      ...
      document.getElementById("gold-time").textContent =
        goldRunner.time.toFixed(2);
      document.getElementById("silver-name").textContent = silverRunner.name;
      document.getElementById("silver-time").textContent =
        silverRunner.time.toFixed(2);
      document.getElementById("bronze-name").textContent = bronzeRunner.name;
      document.getElementById("bronze-time").textContent =
        bronzeRunner.time.toFixed(2);
      ...
      runnerUps.forEach((runner) => {
        ...
        listItem.textContent = `${runner.name} - time: ${runner.time.toFixed(
          2
        )}`;
